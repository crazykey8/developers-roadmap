...
Куки (Cookies) — это небольшие фрагменты данных, которые веб-сервер отправляет браузеру пользователя и которые затем сохраняются на стороне клиента. Эти данные отправляются обратно серверу при каждом последующем запросе к тому же домену.
...
- Ограничены по размеру (около 4 КБ на один cookie).
- Могут быть доступны как на фронтенде (через JavaScript), так и на бэкенде.
- Имеют свойства: `name`, `value`, `expires`, `path`, `domain`, `secure`, `HttpOnly`, `SameSite`.

---

## ⚙️ Где можно использовать куки на Frontend

### 1. **Аутентификация и хранение токенов**

- **Пример**: Сохраняем JWT токен в cookie с флагом `HttpOnly` — безопаснее, чем `localStorage`, так как JavaScript не имеет к нему доступа (неуязвим к XSS).
- **Когда**: Если сервер обрабатывает сессионные или токенизированные запросы с cookie (например, `Authorization` в cookie), а не через заголовки.

```js
// Запись cookie (безопасно использовать только для неважных данных)
document.cookie = "theme=dark; path=/; max-age=3600";
```

---

### 2. **Запоминание пользовательских настроек**

- Темная тема, язык, размер шрифта.
- Пример: сохранить выбранную пользователем тему
...
### 3. **Отслеживание сессии или пользовательского поведения**

- Например, Google Analytics или собственные метрики.
- Пример: сохранить уникальный идентификатор сессии

```js
document.cookie = `sessionId=${crypto.randomUUID()}; path=/; max-age=1800`;
```
...
- Cookie может хранить вариант теста (A или B), чтобы пользователь всегда видел одну и ту же версию.

```js
// Пример выбора случайного варианта и сохранения его
const variant = Math.random() < 0.5 ? "A" : "B";
document.cookie = `ab_test=${variant}; path=/; max-age=604800`;
```

---

### 5. **Обработка баннеров согласия на cookies (GDPR)**

- Сохраняем, согласился ли пользователь на использование cookies.
...
| Флаг                  | Назначение                                                  |
| --------------------- | ----------------------------------------------------------- |
| `HttpOnly`            | Доступен только серверу. Защита от XSS.                     |
| `Secure`              | Только по HTTPS.                                            |
| `SameSite`            | Защита от CSRF (`Lax`, `Strict`, `None`).                   |
| `max-age` / `expires` | Время жизни cookie. Без него — cookie считается сессионным. |

---

## 📦 Альтернативы

| Хранилище          | Доступ из JS                | Безопасность                             | Объем     |
| ------------------ | --------------------------- | ---------------------------------------- | --------- |
| **Cookies**        | Частично (`HttpOnly` — нет) | Средняя                                  | \~4 КБ    |
| **localStorage**   | Да                          | Уязвим к XSS                             | \~5–10 МБ |
| **sessionStorage** | Да                          | Уязвим к XSS, живёт пока вкладка открыта | \~5 МБ    |

---

## ✅ Краткие кейсы по применению на фронте

| Кейс                            | Применимо ли cookie  | Почему                                          |
| ------------------------------- | -------------------- | ----------------------------------------------- |
| Хранение access token           | ✅ (если `HttpOnly`) | Безопаснее, чем `localStorage`                  |
| Запоминание языка               | ✅                   | Простая настройка                               |
| Счётчик посещений               | ✅                   | Легко реализовать                               |
| Большие пользовательские данные | ❌                   | Лучше использовать `localStorage` или IndexedDB |
| Настройки оформления            | ✅                   | Кросс-сессионное хранение                       |

---

Хочешь — могу привести готовый JS-утилитный класс для работы с куками или React hook для чтения/записи cookie.
